/**
 * Import IFC Season 7 Opponent History to MongoDB
 * This script updates fighter documents with Season 7 opponent history data
...
/**
 * Load Season 7 opponent history data
 */
function loadOpponentHistoryData() {
  const dataPath = path.join(__dirname, '../../old-data/season7-opponent-history.json');
 ...
 * Combines existing opponent history with Season 7 data
 */
function mergeOpponentHistory(existingHistory, season7History) {
  if (!existingHistory || existingHistory.length === 0) {
    // No existing history, return Season 7 history as-is
    return season7History;
  }
  ...
  // Merge Season 7 data
  season7History.forEach(season7Opp => {
    const opponentIdStr = season7Opp.opponentId.toString();
    
    if (existingMap.has(opponentIdStr)) {
      // Opponent exists, merge data
      const existing = existingMap.get(opponentIdStr);
      existing.totalFights += season7Opp.totalFights;
      existing.totalWins += season7Opp.totalWins;
      existing.totalLosses += season7Opp.totalLosses;
      existing.winPercentage = Math.round((existing.totalWins / existing.totalFights) * 100);
      
      // Add Season 7 details
      existing.details = existing.details || [];
      existing.details.push(...season7Opp.details);
    } else {
      // New opponent, add to map
      existingMap.set(opponentIdStr, {
        opponentId: season7Opp.opponentId,
        totalFights: season7Opp.totalFights,
        totalWins: season7Opp.totalWins,
        totalLosses: season7Opp.totalLosses,
        winPercentage: season7Opp.winPercentage,
        details: season7Opp.details
      });
    }
  });
...
/**
 * Import Season 7 opponent history to database
 */
async function importOpponentHistory() {
  console.log('\n' + '='.repeat(70));
  console.log('IFC SEASON 7 - OPPONENT HISTORY IMPORT TO MONGODB');
  ...
  try {
    ...
    console.log('\nThis script will UPDATE fighter documents with Season 7 opponent history.');
    console.log('Existing opponent history will be MERGED with Season 7 data.');
    ...
    const fightersWithSeason7 = await Fighter.countDocuments({
    ...
    console.log(`\n📊 Database Statistics:`);
    console.log(`  - Fighters with Season 7 opponent history: ${fightersWithSeason7}`);

    console.log('\n' + '='.repeat(70));
    console.log('✨ IMPORT SUCCESSFUL! ✨');
    console.log('='.repeat(70));
    console.log('\nSeason 7 opponent history has been imported to MongoDB!');
    console.log('\n📋 Summary:');
