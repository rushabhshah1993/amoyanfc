/**
 * Import IFC Season 9 Competition History to MongoDB
 * This script updates fighter documents with Season 9 competition history data
 * It adds Season 9 to seasonDetails and updates overall competition statistics
 */
...
/**
 * Load Season 9 competition history data
 */
function loadCompetitionHistoryData() {
  const dataPath = path.join(__dirname, '../../old-data/season9-competition-history.json');
...
/**
 * Get Season 9 competition ObjectId
 */
async function getSeason9CompetitionId() {
...
/**
 * Import Season 9 competition history to database
 */
async function importCompetitionHistory() {
  console.log('\n' + '='.repeat(70));
  console.log('IFC SEASON 9 - COMPETITION HISTORY IMPORT TO MONGODB');
  ...
  try {
    ...
    // Get Season 9 competition ID
    console.log('\n📂 Loading Season 9 competition...');
    const season9CompetitionId = await getSeason9CompetitionId();
    
    if (!season9CompetitionId) {
      console.error('❌ Season 9 competition not found in database!');
      console.error('   Please import Season 9 competition data first.');
      return;
    }
    
    console.log(`✅ Found Season 9 competition: ${season9CompetitionId}`);

    console.log('\n' + '='.repeat(70));
    console.log('IMPORT OPTIONS');
    console.log('='.repeat(70));
    
    // Check for existing Season 9 data
    ...
    if (existingCount > 0) {
      console.log(`\n⚠️  WARNING: Found ${existingCount} fighters with existing Season 9 competition history!`);
      console.log('   This script will UPDATE existing Season 9 data.');
      console.log('   Press Ctrl+C within 5 seconds to cancel...');
      await new Promise(resolve => setTimeout(resolve, 5000));
    } else {
      console.log('\n✅ No existing Season 9 competition history found. Proceeding with fresh import...');
    ...
    for (const fighterData of historyData) {
      try {
        ...
        // Check if Season 9 already exists in seasonDetails
        const existingSeason9Index = competitionEntry.seasonDetails.findIndex(
        ...
        if (existingSeason9Index >= 0) {
          // Update existing Season 9 data
          const oldData = competitionEntry.seasonDetails[existingSeason9Index];
          
          // Subtract old Season 9 stats from totals
          ...
          // Replace with new Season 9 data
          competitionEntry.seasonDetails[existingSeason9Index] = season9Detail;
        } else {
          // Add new Season 9 data
          competitionEntry.seasonDetails.push(season9Detail);
          competitionEntry.numberOfSeasonAppearances++;
        }
        
        // Add Season 9 stats to totals
    ...
    // Count fighters with Season 9 data
    ...
    console.log(`\n✅ Fighters with Season 9 competition history: ${verifiedCount}`);

    // Sample verification - show division winners
    console.log('\n🏆 Division Winners Verification:');
    
    // Division 1 winner: Hetal Boricha (F010)
    const div1Winner = historyData.find(f => f.fighterCode === 'F010');
    if (div1Winner) {
      ...
      if (fighter) {
        ...
        const season9Detail = compHistory?.seasonDetails.find(sd => sd.seasonNumber === SEASON_NUMBER);
        
        console.log(`\n✓ Division 1: ${fighter.firstName} ${fighter.lastName}`);
        console.log(`  - Season 9: ${season9Detail.wins}W-${season9Detail.losses}L, ${season9Detail.points} pts, Position ${season9Detail.finalPosition}`);
    ...
    // Division 2 winner: Rushika Mangrola (F028)
    const div2Winner = historyData.find(f => f.fighterCode === 'F028');
    if (div2Winner) {
    ...
    // Division 3 winner: Hinal Parekh (F011)
    const div3Winner = historyData.find(f => f.fighterCode === 'F011');
    if (div3Winner) {
    ...
    console.log('='.repeat(70));
    console.log('\nSeason 9 competition history has been imported to MongoDB!');
    console.log('\n📋 Summary:');
