/**
 * Import IFC Season 9 Opponent History to MongoDB
 * This script updates fighter documents with Season 9 opponent history data
...
/**
 * Load Season 9 opponent history data
 */
function loadOpponentHistoryData() {
  const dataPath = path.join(__dirname, '../../old-data/season9-opponent-history.json');
 ...
 * Combines existing opponent history with Season 9 data
 */
function mergeOpponentHistory(existingHistory, season9History) {
  if (!existingHistory || existingHistory.length === 0) {
    // No existing history, return Season 9 history as-is
    return season9History;
  }
  ...
  // Merge Season 9 data
  season9History.forEach(season9Opp => {
    const opponentIdStr = season9Opp.opponentId.toString();
    
    if (existingMap.has(opponentIdStr)) {
      // Opponent exists, merge data
      const existing = existingMap.get(opponentIdStr);
      existing.totalFights += season9Opp.totalFights;
      existing.totalWins += season9Opp.totalWins;
      existing.totalLosses += season9Opp.totalLosses;
      existing.winPercentage = Math.round((existing.totalWins / existing.totalFights) * 100);
      
      // Add Season 9 details
      existing.details = existing.details || [];
      existing.details.push(...season9Opp.details);
    } else {
      // New opponent, add to map
      existingMap.set(opponentIdStr, {
        opponentId: season9Opp.opponentId,
        totalFights: season9Opp.totalFights,
        totalWins: season9Opp.totalWins,
        totalLosses: season9Opp.totalLosses,
        winPercentage: season9Opp.winPercentage,
        details: season9Opp.details
      });
    }
  });
...
/**
 * Import Season 9 opponent history to database
 */
async function importOpponentHistory() {
  console.log('\n' + '='.repeat(70));
  console.log('IFC SEASON 9 - OPPONENT HISTORY IMPORT TO MONGODB');
  ...
  try {
    ...
    console.log('\nThis script will UPDATE fighter documents with Season 9 opponent history.');
    console.log('Existing opponent history will be MERGED with Season 9 data.');
    ...
    for (const fighterData of sampleFighters) {
      ...
      if (fighter) {
        ...
        if (fighter.opponentsHistory && fighter.opponentsHistory.length > 0) {
          ...
          // Check for Season 9 data
          const season9Details = sample.details?.filter(d => d.season === 9);
          if (season9Details && season9Details.length > 0) {
            console.log(`    Season 9 fights: ${season9Details.length}`);
            console.log(`    Sample Fight ID: ${season9Details[0].fightId}`);
          }
        }
      }
    }
    ...
    const fightersWithSeason9 = await Fighter.countDocuments({
    ...
    console.log(`\n📊 Database Statistics:`);
    console.log(`  - Fighters with Season 9 opponent history: ${fightersWithSeason9}`);

    console.log('\n' + '='.repeat(70));
    console.log('✨ IMPORT SUCCESSFUL! ✨');
    console.log('='.repeat(70));
    console.log('\nSeason 9 opponent history has been imported to MongoDB!');
    console.log('\n📋 Summary:');
