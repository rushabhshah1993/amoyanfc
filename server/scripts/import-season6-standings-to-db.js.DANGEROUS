/**
 * Import IFC Season 6 Round Standings to MongoDB
 * This script imports the calculated standings for every fight in Season 6
 */
...
/**
 * Load Season 6 standings from JSON file
 */
function loadSeason6Standings() {
  const dataPath = path.join(__dirname, '../../old-data/migrated-standings/season6-all-rounds-standings.json');
  ...
  console.log(`✅ Loaded ${standingsData.length} standing snapshots for Season 6`);
  
  return standingsData;
}

/**
 * Import Season 6 standings to database
 */
async function importSeason6Standings() {
  console.log('\n' + '='.repeat(70));
  console.log('IFC SEASON 6 - STANDINGS IMPORT TO MONGODB');
  ...
  try {
    ...
    // Load standings data
    const standingsData = loadSeason6Standings();

    console.log('\n' + '='.repeat(70));
    console.log('IMPORT OPTIONS');
    console.log('='.repeat(70));

    // Check if Season 6 standings already exist
    const existingCount = await RoundStandings.countDocuments({
      seasonNumber: 6
    });

    if (existingCount > 0) {
      console.log(`\n⚠️  WARNING: Found ${existingCount} existing standings for Season 6!`);
      console.log('   This script will DELETE existing Season 6 standings and import fresh data.');
      console.log('   Press Ctrl+C within 5 seconds to cancel...');
      
      // Wait 5 seconds
      await new Promise(resolve => setTimeout(resolve, 5000));
      
      console.log('\n🗑️  Deleting existing Season 6 standings...');
      const deleteResult = await RoundStandings.deleteMany({
        seasonNumber: 6
      });
      console.log(`✅ Deleted ${deleteResult.deletedCount} existing Season 6 standings`);
    }
    ...
    console.log('✅ Successfully imported all Season 6 standings!');
    ...
    console.log(`\n✅ Season 6 standings verified in database:`);
    ...
    for (let div = 1; div <= 3; div++) {
      const lastRound = div === 1 ? 9 : (div === 2 ? 11 : 15);
    ...
    console.log('='.repeat(70));
    console.log('\nSeason 6 standings have been imported to MongoDB!');
