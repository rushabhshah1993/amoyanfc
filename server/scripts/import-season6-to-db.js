/**
 * Import IFC Season 6 Competition Data to MongoDB
 * This script imports the migrated Season 6 data into the database
 */
...
/**
 * Load Season 6 data from JSON file
 */
function loadSeason6Data() {
  const dataPath = path.join(__dirname, '../../old-data/ifc-season6-migrated.json');
  ...
  console.log(`✅ Loaded Season 6 data with ${seasonData.leagueData.divisions.length} divisions`);
  
  return seasonData;
}

/**
 * Import Season 6 data to database
 */
async function importSeason6Data() {
  console.log('\n' + '='.repeat(70));
  console.log('IFC SEASON 6 - COMPETITION DATA IMPORT TO MONGODB');
  ...
  try {
    ...
    // Load Season 6 data
    const seasonData = loadSeason6Data();

    console.log('\n' + '='.repeat(70));
    console.log('IMPORT OPTIONS');
    console.log('='.repeat(70));

    // Check if Season 6 data already exists
    ...
    if (existingCompetition) {
      console.log(`\n⚠️  WARNING: Found existing competition data for Season 6!`);
      console.log(`   Existing Season ID: ${existingCompetition._id}`);
      console.log('   This script will DELETE the existing Season 6 and import fresh data.');
      console.log('   Press Ctrl+C within 5 seconds to cancel...');
      
      // Wait 5 seconds
      await new Promise(resolve => setTimeout(resolve, 5000));
      
      console.log('\n🗑️  Deleting existing Season 6...');
      await Competition.findByIdAndDelete(existingCompetition._id);
      console.log('✅ Deleted existing Season 6 document');
    }
    ...
    console.log('\n📥 Creating Season 6 competition document...');
    
    const newCompetition = new Competition(seasonData);
    const savedCompetition = await newCompetition.save();
    
    console.log('✅ Season 6 competition data imported successfully!');
    ...
    console.log(`\n✅ Season 6 data verified in database:`);
    ...
    console.log('='.repeat(70));
    console.log('\nSeason 6 competition data has been imported to MongoDB!');
    console.log(`Document ID: ${savedCompetition._id}`);
    console.log('\nNext step: Import Season 6 standings data using:');
    console.log('  npm run import:season6:standings');
    console.log('');
