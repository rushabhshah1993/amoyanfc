/**
 * Import IFC Season 7 Streaks to MongoDB
 * This script updates fighter documents with Season 7 streaks data
 */
...
/**
 * Load Season 7 streaks data
 */
function loadStreaksData() {
  const dataPath = path.join(__dirname, '../../old-data/season7-streaks-updates.json');
...
/**
 * Import Season 7 streaks to database
 */
async function importStreaks() {
  console.log('\n' + '='.repeat(70));
  console.log('IFC SEASON 7 - STREAKS IMPORT TO MONGODB');
  ...
  try {
    ...
    console.log('\n⚠️  WARNING: This will REPLACE all streaks for Season 7 fighters!');
    console.log('   The new data includes:');
    console.log('   - Updated streaks from previous seasons');
    console.log('   - New streaks from Season 7');
    console.log('   - Active streaks after Season 7');
    ...
    for (const fighterData of sampleFighters) {
      ...
      if (fighter) {
        ...
        if (activeStreak) {
          ...
          // Check if it's a Season 7 streak
          if (activeStreak.start.season === SEASON_NUMBER) {
            console.log(`  - ✨ Started in Season 7`);
          } else {
            console.log(`  - 🔗 Continued from Season ${activeStreak.start.season}`);
          }
        }
        
        // Count streaks from Season 7
        const season7Streaks = fighter.streaks?.filter(s => 
          s.start.season === SEASON_NUMBER || (s.end && s.end.season === SEASON_NUMBER)
        ) || [];
        
        if (season7Streaks.length > 0) {
          console.log(`  - Season 7 streaks: ${season7Streaks.length}`);
        }
      }
    }
    ...
    console.log('='.repeat(70));
    console.log('\nSeason 7 streaks have been imported to MongoDB!');
    console.log('\n📋 Summary:');
