/**
 * Import IFC Season 9 Streaks to MongoDB
 * This script updates fighter documents with Season 9 streaks data
 */
...
/**
 * Load Season 9 streaks data
 */
function loadStreaksData() {
  const dataPath = path.join(__dirname, '../../old-data/season9-streaks-updates.json');
...
/**
 * Import Season 9 streaks to database
 */
async function importStreaks() {
  console.log('\n' + '='.repeat(70));
  console.log('IFC SEASON 9 - STREAKS IMPORT TO MONGODB');
  ...
  try {
    ...
    console.log('\n⚠️  WARNING: This will REPLACE all streaks for Season 9 fighters!');
    console.log('   The new data includes:');
    console.log('   - Updated streaks from previous seasons');
    console.log('   - New streaks from Season 9');
    console.log('   - Active streaks after Season 9');
    ...
    const rushika = streaksData.find(f => f.fighterName === 'Rushika Mangrola');
    const tanvi = streaksData.find(f => f.fighterName === 'Tanvi Shah');
    
    const notableFighters = [rushika, tanvi].filter(Boolean);
    ...
    for (const fighterData of notableFighters) {
      ...
      if (fighter) {
        ...
        if (activeStreak) {
          ...
          // Check if it's a Season 9 streak
          if (activeStreak.start.season === SEASON_NUMBER) {
            console.log(`  - ✨ Started in Season 9`);
          } else {
            console.log(`  - 🔗 Continued from Season ${activeStreak.start.season}`);
          }
        }
        
        // Count streaks from Season 9
        const season9Streaks = fighter.streaks?.filter(s => 
          s.start.season === SEASON_NUMBER || (s.end && s.end.season === SEASON_NUMBER)
        ) || [];
        
        if (season9Streaks.length > 0) {
          console.log(`  - Season 9 streaks: ${season9Streaks.length}`);
        }
      }
    }
    ...
    console.log('='.repeat(70));
    console.log('\nSeason 9 streaks have been imported to MongoDB!');
    console.log('\n📋 Summary:');
    ...
    console.log(`   - Longest active win streak: 8 fights (Rushika Mangrola)`);
