/**
 * Calculate Season 7 Streaks Data
 * This script processes Season 7 fights and updates fighter streaks
 * continuing from the active streaks in Season 6 data
 */
...
/**
 * Calculate streaks for Season 7
 */
async function calculateSeason7Streaks() {
  console.log('\n' + '='.repeat(70));
  console.log('SEASON 7 STREAKS CALCULATION');
  ...
  try {
    ...
    console.log(`✅ Loaded Season 7 with ${season7Data.leagueData.divisions.length} divisions`);
    
    // Initialize fighter map
    const fighterMap = initializeFighterMap(fightersData);
    console.log(`✅ Initialized fighter map`);
    
    // Get fighters participating in Season 7
    const season7FighterIds = new Set();
    season7Data.leagueData.divisions.forEach(division => {
      division.rounds.forEach(round => {
        round.fights.forEach(fight => {
          if (fight.fighter1 && fight.fighter2) {
            season7FighterIds.add(fight.fighter1);
            season7FighterIds.add(fight.fighter2);
          }
        });
      });
    });
    
    console.log(`\n📊 Processing Season 7 fights...`);
    console.log(`   - Total fighters in Season 7: ${season7FighterIds.size}`);
    ...
    // Prepare output data (only for Season 7 fighters)
    const updatedFighters = [];
    
    season7FighterIds.forEach(fighterId => {
    ...
    // Save to file
    const outputPath = path.join(__dirname, '../../old-data/season7-streaks-updates.json');
    ...
    let totalClosedStreaksInSeason7 = 0;
    ...
    console.log(`   - Streaks closed in Season 7: ${totalClosedStreaksInSeason7}`);
    ...
    console.log('='.repeat(70));
    console.log('\nNext step: Verify the streaks data using:');
    console.log('  node server/scripts/verify-season7-streaks.js');
    console.log('');
