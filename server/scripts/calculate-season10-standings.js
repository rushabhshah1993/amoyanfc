/**
 * Calculate Season 10 Round Standings
 * This script processes all fights in Season 10 and calculates standings after each fight
 ...
 * Format: IFC-S10-D1-R1-F1
 */
function parseFightIdentifier(fightIdentifier) {
  const parts = fightIdentifier.split('-');
  
  if (parts.length !== 5 || parts[0] !== 'IFC') {
    throw new Error(`Invalid fight identifier format: ${fightIdentifier}. Expected format: IFC-S10-D1-R1-F1`);
  }
...
function calculateAllStandings() {
  console.log('\n' + '='.repeat(70));
  console.log('SEASON 10 STANDINGS CALCULATION');
  ...
  try {
    ...
    season10Data.leagueData.divisions.forEach(division => {
      ...
      const divisionFighters = season10Data.seasonMeta.leagueDivisions
    ...
    const outputPath = path.join(__dirname, '../../old-data/migrated-standings/season10-all-rounds-standings.json');
    ...
    console.log('  old-data/migrated-standings/season10-all-rounds-standings.json');
    console.log('');
