/**
 * Calculate Season 9 Round Standings
 * This script processes all fights in Season 9 and calculates standings after each fight
 ...
 * Format: IFC-S9-D1-R1-F1
 */
function parseFightIdentifier(fightIdentifier) {
  const parts = fightIdentifier.split('-');
  
  if (parts.length !== 5 || parts[0] !== 'IFC') {
    throw new Error(`Invalid fight identifier format: ${fightIdentifier}. Expected format: IFC-S9-D1-R1-F1`);
  }
 ...
 * 3. Alphabetical by first name (ascending)
 */
...
function calculateAllStandings() {
  console.log('\n' + '='.repeat(70));
  console.log('SEASON 9 STANDINGS CALCULATION');
  ...
  try {
    ...
    season9Data.leagueData.divisions.forEach(division => {
      ...
      const divisionFighters = season9Data.seasonMeta.leagueDivisions
    ...
    const outputPath = path.join(__dirname, '../../old-data/migrated-standings/season9-all-rounds-standings.json');
    ...
    console.log('='.repeat(70));
    console.log('\nNext step: Review the standings data at:');
    console.log('  old-data/migrated-standings/season9-all-rounds-standings.json');
    console.log('');
