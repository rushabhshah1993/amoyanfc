/**
 * Import Season 9 Standings to MongoDB
 * This script imports all standing snapshots for Season 9 into the database
 */
...
/**
 * Load Season 9 standings from JSON file
 */
function loadSeason9Standings() {
  const dataPath = path.join(__dirname, '../../old-data/migrated-standings/season9-all-rounds-standings.json');
...
/**
 * Import Season 9 standings to database
 */
async function importSeason9Standings() {
  console.log('\n' + '='.repeat(70));
  console.log('IFC SEASON 9 - STANDINGS IMPORT TO MONGODB');
  ...
  try {
    ...
    // Load Season 9 standings
    const standings = loadSeason9Standings();

    console.log('\n' + '='.repeat(70));
    console.log('CHECKING EXISTING DATA');
    console.log('='.repeat(70));

    // Check if Season 9 standings already exist
    const existingCount = await RoundStandings.countDocuments({ seasonNumber: 9 });

    if (existingCount > 0) {
      console.log(`\n⚠️  WARNING: Found ${existingCount} existing standings for Season 9!`);
      console.log('   This script will DELETE all existing Season 9 standings and import fresh data.');
      console.log('   Press Ctrl+C within 5 seconds to cancel...');
      
      // Wait 5 seconds
      await new Promise(resolve => setTimeout(resolve, 5000));
      
      console.log('\n🗑️  Deleting existing Season 9 standings...');
      const deleteResult = await RoundStandings.deleteMany({ seasonNumber: 9 });
      console.log(`✅ Deleted ${deleteResult.deletedCount} existing standings`);
    } else {
      console.log('\n✅ No existing Season 9 standings found. Proceeding with fresh import...');
    }
    ...
    const verifyCount = await RoundStandings.countDocuments({ seasonNumber: 9 });
    ...
    for (let divisionNumber = 1; divisionNumber <= 3; divisionNumber++) {
      ...
      const fightIdentifier = `IFC-S9-D${divisionNumber}-R${lastRound}-F${lastFight}`;
    ...
    console.log('='.repeat(70));
    console.log('\nSeason 9 standings have been imported to MongoDB!');
    console.log('\n📋 Summary:');
