/**
 * Calculate Season 8 Round Standings
 * This script processes all fights in Season 8 and calculates standings after each fight
...
function parseFightIdentifier(fightIdentifier) {
  ...
  if (parts.length !== 4) {
    throw new Error(`Invalid fight identifier format: ${fightIdentifier}. Expected format: S8-D1-R1-F1`);
  }
...
function calculateAllStandings() {
  console.log('\n' + '='.repeat(70));
  console.log('SEASON 8 STANDINGS CALCULATION');
  ...
  try {
    ...
    console.log(`\n✅ Loaded Season 8 with ${season8Data.leagueData.divisions.length} divisions`);
    ...
    season8Data.leagueData.divisions.forEach(division => {
      ...
      const divisionFighters = season8Data.seasonMeta.leagueDivisions
    ...
    const outputPath = path.join(__dirname, '../../old-data/migrated-standings/season8-all-rounds-standings.json');
    ...
    console.log('  old-data/migrated-standings/season8-all-rounds-standings.json');
    console.log('');
