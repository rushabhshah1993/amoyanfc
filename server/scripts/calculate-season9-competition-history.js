/**
 * Calculate Season 9 Competition History
 * This script calculates competition statistics for all Season 9 fighters
...
/**
 * Load Season 9 migrated data
 */
function loadSeason9Data() {
  const migratedPath = path.join(__dirname, '../../old-data/ifc-season9-migrated.json');
  
  console.log(`📂 Loading Season 9 migrated data from: ${migratedPath}`);
...
/**
 * Load Season 9 standings data
 */
function loadStandingsData() {
  const standingsPath = path.join(__dirname, '../../old-data/migrated-standings/season9-all-rounds-standings.json');
  
  console.log(`📂 Loading Season 9 standings from: ${standingsPath}`);
...
function getFinalPositionsFromStandings(standingsData) {
  ...
  const div1Final = standingsData.find(s => 
    s.divisionNumber === 1 && 
    s.roundNumber === 9 && 
    s.fightIdentifier === 'IFC-S9-D1-R9-F5'
  );
  ...
  const div2Final = standingsData.find(s => 
    s.divisionNumber === 2 && 
    s.roundNumber === 11 && 
    s.fightIdentifier === 'IFC-S9-D2-R11-F6'
  );
  ...
  const div3Final = standingsData.find(s => 
    s.divisionNumber === 3 && 
    s.roundNumber === 15 && 
    s.fightIdentifier === 'IFC-S9-D3-R15-F8'
  );
...
/**
 * Calculate Season 9 statistics for all fighters
 */
function calculateSeason9Statistics(seasonData, standingsData, fightersData) {
  console.log('\n📊 Calculating Season 9 competition history...');
  ...
  // Initialize all Season 9 fighters
  ...
  console.log(`   - Total fighters in Season 9: ${fighterStats.size}`);
...
function calculateCompetitionHistory() {
  console.log('\n' + '='.repeat(70));
  console.log('SEASON 9 COMPETITION HISTORY CALCULATION');
  console.log('='.repeat(70));

  try {
    const season9Data = loadSeason9Data();
    const standingsData = loadStandingsData();
    const fightersData = loadFightersData();
    
    console.log(`\n✅ Loaded Season 9 with ${season9Data.leagueData.divisions.length} divisions`);
    console.log(`✅ Loaded ${standingsData.length} standings snapshots`);
    
    const season9Stats = calculateSeason9Statistics(season9Data, standingsData, fightersData);
    
    console.log('\n' + '='.repeat(70));
    console.log('CALCULATION SUMMARY');
    console.log('='.repeat(70));
    
    console.log(`\n✅ Calculated competition history for ${season9Stats.length} fighters`);
    
    // Show sample data
    console.log('\n📋 Sample Competition History (First 5 fighters):');
    season9Stats.slice(0, 5).forEach(fighter => {
    ...
    [1, 2, 3].forEach(divNum => {
      const divFighters = season9Stats.filter(f => f.divisionNumber === divNum);
    ...
    [1, 2, 3].forEach(divNum => {
      const winner = season9Stats.find(f => f.divisionNumber === divNum && f.finalPosition === 1);
      if (winner) {
    ...
    // Save to file
    const outputPath = path.join(__dirname, '../../old-data/season9-competition-history.json');
    fs.writeFileSync(outputPath, JSON.stringify(season9Stats, null, 2), 'utf8');
    ...
    console.log('='.repeat(70));
    console.log('\nSeason 9 competition history has been calculated!');
    console.log('\nNext steps:');
    console.log('1. Review the competition history data');
    console.log('2. Verify final positions match standings');
    console.log('3. Import to MongoDB using import-season9-competition-history.js');
    console.log('');
