/**
 * Calculate Season 10 Opponent History with Fight ObjectIds
 * This script processes all fights in Season 10 and calculates opponent history
...
/**
 * Load Season 10 competition from database
 */
async function loadSeason10FromDB() {
  ...
  if (!competition) {
    throw new Error('Season 10 competition not found in database');
  }
  
  console.log(`✅ Loaded Season 10 with ${competition.leagueData.divisions.length} divisions`);
...
function processSeasonFights(seasonData, fightersData) {
  console.log('\n📊 Processing Season 10 fights for opponent history...');
  ...
  console.log(`   - Total fighters in Season 10: ${allFighterIds.size}`);
  ...
  console.log(`\n✅ Processed ${totalFightsProcessed} total fights`);
...
async function calculateOpponentHistory() {
  console.log('\n' + '='.repeat(70));
  console.log('SEASON 10 OPPONENT HISTORY CALCULATION (WITH FIGHT IDS)');
  ...
  try {
    ...
    // Load Season 10 from database (to get fight ObjectIds)
    const season10Data = await loadSeason10FromDB();
    const fightersData = loadFightersData();
    
    const opponentHistoryData = processSeasonFights(season10Data, fightersData);
    ...
    if (rematchData.length > 0) {
      ...
      console.log('\n🔄 Rematches in Season 10:');
      rematchData.slice(0, 10).forEach(match => {
        console.log(`   - ${match.fighter1} vs ${match.fighter2}: ${match.totalFights} fights (${match.record})`);
      });
    }
    
    // Save to file
    const outputPath = path.join(__dirname, '../../old-data/season10-opponent-history.json');
    ...
    console.log('='.repeat(70));
    console.log('\nOpponent history has been calculated for Season 10 with fight ObjectIds!');
    console.log('\nNext steps:');
    console.log('1. Review the opponent history data');
    console.log('2. Verify fight ObjectIds are correct');
    console.log('3. Import to MongoDB using import-season10-opponent-history.js');
    console.log('');
