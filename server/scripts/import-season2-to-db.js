/**
 * Import IFC Season 2 Competition Data to MongoDB
...
function loadSeason2Data() {
  ...
  console.log(`   - Season Number: ${season2Data.seasonMeta.seasonNumber}`);
  console.log(`   - Divisions: ${season2Data.leagueData.divisions.length}`);
  console.log(`   - Total Rounds: ${season2Data.leagueData.divisions[0]?.totalRounds || 0}`);
  
  return season2Data;
}

/**
 * Import Season 2 to database
 */
async function importSeason2() {
  console.log('\n' + '='.repeat(70));
  console.log('IFC SEASON 2 - COMPETITION DATA IMPORT TO MONGODB');
  ...
  try {
    ...
    if (existingSeason) {
      console.log('\n⚠️  WARNING: Season 2 already exists in the database!');
      console.log(`   Existing Season ID: ${existingSeason._id}`);
      console.log('   This script will DELETE the existing Season 2 and import fresh data.');
      console.log('   Press Ctrl+C within 5 seconds to cancel...');
      
      // Wait 5 seconds
      await new Promise(resolve => setTimeout(resolve, 5000));
      
      console.log('\n🗑️  Deleting existing Season 2...');
      await Competition.findByIdAndDelete(existingSeason._id);
      console.log('✅ Deleted existing Season 2 document');
    }
    ...
    if (!verifiedSeason) {
      throw new Error('Season 2 document not found after import!');
    }
    ...
    console.log('='.repeat(70));
    console.log('\nSeason 2 has been imported to MongoDB!');
    console.log('You can now query Season 2 data using:');
