/**
 * Calculate Season 9 Opponent History with Fight ObjectIds
 * This script processes all fights in Season 9 and calculates opponent history
...
/**
 * Load Season 9 competition from database
 */
async function loadSeason9FromDB() {
  ...
  if (!competition) {
    throw new Error('Season 9 competition not found in database');
  }
  
  console.log(`✅ Loaded Season 9 with ${competition.leagueData.divisions.length} divisions`);
...
function processSeasonFights(seasonData, fightersData) {
  console.log('\n📊 Processing Season 9 fights for opponent history...');
  ...
  console.log(`   - Total fighters in Season 9: ${allFighterIds.size}`);
...
async function calculateOpponentHistory() {
  console.log('\n' + '='.repeat(70));
  console.log('SEASON 9 OPPONENT HISTORY CALCULATION (WITH FIGHT IDS)');
  ...
  try {
    ...
    // Load Season 9 from database (to get fight ObjectIds)
    const season9Data = await loadSeason9FromDB();
    const fightersData = loadFightersData();
    
    const opponentHistoryData = processSeasonFights(season9Data, fightersData);
    ...
    // Save to file
    const outputPath = path.join(__dirname, '../../old-data/season9-opponent-history.json');
    ...
    console.log('='.repeat(70));
    console.log('\nOpponent history has been calculated for Season 9 with fight ObjectIds!');
    console.log('\nNext steps:');
    console.log('1. Review the opponent history data');
    console.log('2. Verify fight ObjectIds are correct');
    console.log('3. Import to MongoDB using import-season9-opponent-history.js');
    console.log('');
