/**
 * Import IFC Season 8 Competition Data to MongoDB
 * This script imports the migrated Season 8 data into the database
 */
...
/**
 * Load Season 8 data from JSON file
 */
function loadSeason8Data() {
  const dataPath = path.join(__dirname, '../../old-data/ifc-season8-migrated.json');
  ...
  console.log(`✅ Loaded Season 8 data with ${seasonData.leagueData.divisions.length} divisions`);
  
  return seasonData;
}

/**
 * Import Season 8 data to database
 */
async function importSeason8Data() {
  console.log('\n' + '='.repeat(70));
  console.log('IFC SEASON 8 - COMPETITION DATA IMPORT TO MONGODB');
  ...
  try {
    ...
    // Load Season 8 data
    const seasonData = loadSeason8Data();

    console.log('\n' + '='.repeat(70));
    console.log('IMPORT OPTIONS');
    console.log('='.repeat(70));

    // Check if Season 8 data already exists
    const existingCompetition = await Competition.findOne({
      competitionMetaId: seasonData.competitionMetaId,
      'seasonMeta.seasonNumber': 8
    });

    if (existingCompetition) {
      console.log(`\n⚠️  WARNING: Found existing competition data for Season 8!`);
      console.log(`   Existing Season ID: ${existingCompetition._id}`);
      console.log('   This script will DELETE the existing Season 8 and import fresh data.');
      console.log('   Press Ctrl+C within 5 seconds to cancel...');
      
      // Wait 5 seconds
      await new Promise(resolve => setTimeout(resolve, 5000));
      
      console.log('\n🗑️  Deleting existing Season 8...');
      await Competition.findByIdAndDelete(existingCompetition._id);
      console.log('✅ Deleted existing Season 8 document');
    } else {
      console.log('\n✅ No existing Season 8 data found. Proceeding with fresh import...');
    }
    ...
    console.log('\n📥 Creating Season 8 competition document...');
    
    const newCompetition = new Competition(seasonData);
    const savedCompetition = await newCompetition.save();
    
    console.log('✅ Season 8 competition data imported successfully!');
    ...
    console.log(`\n✅ Season 8 data verified in database:`);
    ...
    console.log('='.repeat(70));
    console.log('\nSeason 8 competition data has been imported to MongoDB!');
    console.log(`Document ID: ${savedCompetition._id}`);
    console.log('\n📋 Summary:');
