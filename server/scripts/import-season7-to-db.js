/**
 * Import IFC Season 7 Competition Data to MongoDB
 * This script imports the migrated Season 7 data into the database
 */
...
/**
 * Load Season 7 data from JSON file
 */
function loadSeason7Data() {
  ...
  console.log(`✅ Loaded Season 7 data with ${seasonData.leagueData.divisions.length} divisions`);
  
  return seasonData;
}

/**
 * Import Season 7 data to database
 */
async function importSeason7Data() {
  console.log('\n' + '='.repeat(70));
  console.log('IFC SEASON 7 - COMPETITION DATA IMPORT TO MONGODB');
  ...
  try {
    ...
    // Load Season 7 data
    const seasonData = loadSeason7Data();

    console.log('\n' + '='.repeat(70));
    console.log('IMPORT OPTIONS');
    console.log('='.repeat(70));

    // Check if Season 7 data already exists
    ...
    if (existingCompetition) {
      console.log(`\n⚠️  WARNING: Found existing competition data for Season 7!`);
      console.log(`   Existing Season ID: ${existingCompetition._id}`);
      console.log('   This script will DELETE the existing Season 7 and import fresh data.');
      console.log('   Press Ctrl+C within 5 seconds to cancel...');
      
      // Wait 5 seconds
      await new Promise(resolve => setTimeout(resolve, 5000));
      
      console.log('\n🗑️  Deleting existing Season 7...');
      await Competition.findByIdAndDelete(existingCompetition._id);
      console.log('✅ Deleted existing Season 7 document');
    } else {
      console.log('\n✅ No existing Season 7 data found. Proceeding with fresh import...');
    }
    ...
    console.log('\n📥 Creating Season 7 competition document...');
    
    const newCompetition = new Competition(seasonData);
    const savedCompetition = await newCompetition.save();
    
    console.log('✅ Season 7 competition data imported successfully!');
    ...
    console.log(`\n✅ Season 7 data verified in database:`);
    ...
    console.log('='.repeat(70));
    console.log('\nSeason 7 competition data has been imported to MongoDB!');
    console.log(`Document ID: ${savedCompetition._id}`);
    console.log('\n📋 Summary:');
