/**
 * Import IFC Season 9 Competition Data to MongoDB
 * This script imports the migrated Season 9 data into the database
 */
...
/**
 * Load Season 9 data from JSON file
 */
function loadSeason9Data() {
  const dataPath = path.join(__dirname, '../../old-data/ifc-season9-migrated.json');
  ...
  console.log(`✅ Loaded Season 9 data with ${seasonData.leagueData.divisions.length} divisions`);
  
  return seasonData;
}

/**
 * Import Season 9 data to database
 */
async function importSeason9Data() {
  console.log('\n' + '='.repeat(70));
  console.log('IFC SEASON 9 - COMPETITION DATA IMPORT TO MONGODB');
  ...
  try {
    ...
    // Load Season 9 data
    const seasonData = loadSeason9Data();

    console.log('\n' + '='.repeat(70));
    console.log('IMPORT OPTIONS');
    console.log('='.repeat(70));

    // Check if Season 9 data already exists
    const existingCompetition = await Competition.findOne({
      competitionMetaId: seasonData.competitionMetaId,
      'seasonMeta.seasonNumber': 9
    });

    if (existingCompetition) {
      console.log(`\n⚠️  WARNING: Found existing competition data for Season 9!`);
      console.log(`   Existing Season ID: ${existingCompetition._id}`);
      console.log('   This script will DELETE the existing Season 9 and import fresh data.');
      console.log('   Press Ctrl+C within 5 seconds to cancel...');
      
      // Wait 5 seconds
      await new Promise(resolve => setTimeout(resolve, 5000));
      
      console.log('\n🗑️  Deleting existing Season 9...');
      await Competition.findByIdAndDelete(existingCompetition._id);
      console.log('✅ Deleted existing Season 9 document');
    } else {
      console.log('\n✅ No existing Season 9 data found. Proceeding with fresh import...');
    }
    ...
    console.log('\n📥 Creating Season 9 competition document...');
    
    const newCompetition = new Competition(seasonData);
    const savedCompetition = await newCompetition.save();
    
    console.log('✅ Season 9 competition data imported successfully!');
    ...
    console.log(`\n✅ Season 9 data verified in database:`);
    ...
    console.log('='.repeat(70));
    console.log('\nSeason 9 competition data has been imported to MongoDB!');
    console.log(`Document ID: ${savedCompetition._id}`);
    console.log('\n📋 Summary:');
