/**
 * SAFE Import IFC Season 5 Round Standings to MongoDB
 * This script ONLY imports Season 5 standings if they don't exist
...
/**
 * Load Season 5 standings from JSON file
 */
function loadSeason5Standings() {
  const dataPath = path.join(__dirname, '../../old-data/migrated-standings/season5-all-rounds-standings.json');
  ...
  console.log(`✅ Loaded ${standingsData.length} standing snapshots for Season 5`);
  
  return standingsData;
}

/**
 * SAFE Import Season 5 standings to database
 * This function will ONLY add missing standings, never delete existing data
 */
async function importSeason5StandingsSafe() {
  console.log('\n' + '='.repeat(70));
  console.log('IFC SEASON 5 - SAFE STANDINGS IMPORT TO MONGODB');
  ...
  try {
    ...
    // Load standings data
    const standingsData = loadSeason5Standings();

    console.log('\n' + '='.repeat(70));
    console.log('CHECKING EXISTING DATA');
    console.log('='.repeat(70));

    // Check if Season 5 standings already exist
    const existingCount = await RoundStandings.countDocuments({
      seasonNumber: 5
    });

    console.log(`\n📊 Existing Season 5 standings: ${existingCount}`);
    console.log(`📊 Standings in file: ${standingsData.length}`);

    if (existingCount >= standingsData.length) {
      console.log('\n✅ Season 5 standings are already complete in the database!');
    ...
    if (missingStandings.length === 0) {
      console.log('✅ All Season 5 standings are already in the database!');
      return;
    }
    ...
    console.log(`✅ Successfully imported ${importedCount} missing Season 5 standings!`);
    ...
    console.log(`\n✅ Season 5 standings verified in database:`);
    ...
    console.log('='.repeat(70));
    console.log('\n🛡️  Season 5 standings have been safely imported!');
