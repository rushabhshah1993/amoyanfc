/**
 * Update IC Titles for Champions
 * Adds title entries for all Invicta Cup champions
 */
...
/**
 * Main function to update IC titles
 */
async function updateICTitles() {
  console.log('\n' + '='.repeat(70));
  console.log('🏆 UPDATING INVICTA CUP TITLES');
  ...
  try {
    ...
    // Fetch all IC seasons to get their MongoDB IDs
    ...
    // Find all fighters with IC competition history
    console.log('\n🔍 Finding IC fighters...');
    const icFighters = await Fighter.find({
      'competitionHistory.competitionId': IC_COMPETITION_META_ID
    });

    console.log(`✅ Found ${icFighters.length} fighters with IC participation`);
    ...
    for (const fighter of icFighters) {
      try {
        ...
        for (const championship of championships) {
          ...
          const existingTitle = icHistory.titles?.details?.find(
        ...
        if (newTitleDetails.length > 0) {
          ...
          icHistory.titles.details.push(...newTitleDetails);
          icHistory.titles.totalTitles = icHistory.titles.details.length;

          // Save fighter
          await fighter.save();
          updateCount++;

          console.log(`   📊 Total IC titles: ${icHistory.titles.totalTitles}`);
        }
    ...
    console.log('='.repeat(70));
    console.log(`Total IC fighters: ${icFighters.length}`);
    ...
    const logFilename = `ic-titles-update-${timestamp}.log`;
    const logPath = path.join(__dirname, '../../backups', logFilename);
    
    let logContent = `IC TITLES UPDATE LOG\n`;
    logContent += `${'='.repeat(70)}\n\n`;
    logContent += `Update Date: ${new Date().toISOString()}\n`;
    logContent += `Total IC Fighters: ${icFighters.length}\n`;
