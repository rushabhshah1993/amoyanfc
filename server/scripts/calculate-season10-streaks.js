/**
 * Calculate Season 10 Streaks Data
 * This script processes Season 10 fights and updates fighter streaks
 * continuing from the active streaks in Season 9 data
 */
...
/**
 * Load Season 10 competition data from MongoDB
 */
async function loadSeason10Data() {
  console.log(`📂 Loading Season 10 data from MongoDB...`);
  ...
  if (!competition) {
    throw new Error('Season 10 competition not found in MongoDB');
  }
  
  console.log(`✅ Loaded Season 10 competition data`);
  return competition;
}
...
/**
 * Calculate streaks for Season 10
 */
async function calculateSeason10Streaks() {
  console.log('\n' + '='.repeat(70));
  console.log('SEASON 10 STREAKS CALCULATION');
  ...
  try {
    ...
    // Load data
    const season10Data = await loadSeason10Data();
    const fightersData = await loadFightersData();
    
    console.log(`✅ Loaded ${fightersData.length} fighters with existing streaks`);
    console.log(`✅ Loaded Season 10 with ${season10Data.leagueData.divisions.length} divisions`);
    
    // Initialize fighter map
    const fighterMap = initializeFighterMap(fightersData);
    console.log(`✅ Initialized fighter map`);
    
    // Get fighters participating in Season 10
    const season10FighterIds = new Set();
    season10Data.leagueData.divisions.forEach(division => {
      division.rounds.forEach(round => {
        round.fights.forEach(fight => {
          if (fight.fighter1 && fight.fighter2) {
            season10FighterIds.add(fight.fighter1.toString());
            season10FighterIds.add(fight.fighter2.toString());
          }
        });
      });
    });
    
    console.log(`\n📊 Processing Season 10 fights...`);
    console.log(`   - Total fighters in Season 10: ${season10FighterIds.size}`);
    ...
    // Prepare output data (only for Season 10 fighters)
    const updatedFighters = [];
    
    season10FighterIds.forEach(fighterId => {
    ...
    // Save to file
    const outputPath = path.join(__dirname, '../../old-data/season10-streaks-updates.json');
    ...
    let totalClosedStreaksInSeason10 = 0;
    ...
    console.log(`   - Streaks closed in Season 10: ${totalClosedStreaksInSeason10}`);
    ...
    console.log('3. Import to MongoDB using: node server/scripts/import-season10-streaks.js');
    console.log('');
