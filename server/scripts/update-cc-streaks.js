/**
 * Update CC Streaks for All Fighters
 * Processes all CC fights in chronological order and updates win/loss streaks
 */
...
/**
 * Process all CC fights and build streak data
 */
async function processCCStreaks() {
  ...
  console.log(`📊 Processed ${allFights.length} fighter results from ${ccSeasons.length} seasons`);
...
async function updateFighterStreaks(fighterId, newStreaks) {
  ...
  for (const newStreak of newStreaks) {
    // Check if this CC streak already exists
    const existingStreak = fighter.streaks?.find(s => 
      s.competitionId?.toString() === CC_COMPETITION_META_ID &&
    ...
    fighter.streaks.push({
      competitionId: new mongoose.Types.ObjectId(CC_COMPETITION_META_ID),
      type: newStreak.type,
...
/**
 * Main function to update CC streaks
 */
async function updateCCStreaks() {
  console.log('\n' + '='.repeat(70));
  console.log('🔥 UPDATING CC STREAKS');
  ...
  try {
    ...
    // Process all CC fights and calculate streaks
    const fighterStreaks = await processCCStreaks();
    ...
    console.log('='.repeat(70));
    console.log(`Fighters with CC fights: ${fighterStreaks.size}`);
    ...
    const logFilename = `cc-streaks-update-${timestamp}.log`;
    const logPath = path.join(__dirname, '../../backups', logFilename);
    
    let logContent = `CC STREAKS UPDATE LOG\n`;
